-- Fix ambiguous column in smart_reschedule + cap_busy_day, add undo_reschedule,
-- add error_type and time_status columns to error_log.

-- Drop old function versions (return type changed)
drop function if exists smart_reschedule(date);
drop function if exists cap_busy_day(date, int, int);

-- smart_reschedule: rename return col task_text → moved_task_text
...
-- New columns on error_log for the redesigned form
alter table error_log add column if not exists error_type text;
alter table error_log add column if not exists time_status text;
